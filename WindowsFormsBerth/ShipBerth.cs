...
namespace WindowsFormsTransport
{
    ...
    public class Berth<T> where T : class, ITransport
    {
        ...
        public static bool operator +(Berth<T> p, T vehicle)
        {
            ...
            {
                throw new OverflowException("На пристани нет свободных мест");
            }

            foreach (var place in p._places) {
                var comparer = new ShipComparer();
                Vehicle newShip = (Vehicle)(object)place;
                Vehicle ship = (Vehicle)(object)vehicle;

                if (comparer.Compare(ship, newShip) == 0)
                {
                    throw new BerthAlreadyHaveException();
                }
            }
        ...
        public static T operator -(Berth<T> p, int index)
        {
            ...
            {
                throw new IndexOutOfRangeException("Не найден корабль по месту" + index);
            }
        ...
        /// <summary>
        /// Сортировка кораблей на причале
        /// </summary>
