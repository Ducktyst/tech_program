...
using static WindowsFormsBerth.Program;
    ...
    public partial class FormShipConfig : Form
    {
        ...
        private void PanelShip_DragDrop(object sender, DragEventArgs e)
        {
            switch (e.Data.GetData(DataFormats.Text).ToString())
            {
                case "labelWarShip":
                    _ship = new WarShip(
                        (int)numericUpDownMaxSpeed.Value,
        ...
        private void ButtonOk_Click(object sender, EventArgs e)
        {
            ...
            string shipType = _ship.GetType().ToString().Split('.')[1];
            switch (shipType)
            {// Так как при изменении характеристик после добавления корабля на панель они не применялись
                case "WarShip":
                    _ship.SetMaxSpeed((int)numericUpDownMaxSpeed.Value);
                    _ship.SetWeight((int)numericUpDownWeight.Value);
                    break;
                case "Cruiser":
                    Cruiser ship = (Cruiser)_ship;
                    ship.SetMaxSpeed((int)numericUpDownMaxSpeed.Value);
                    ship.SetWeight((int)numericUpDownWeight.Value);
                    ship.IsWithGuns = checkBoxGuns.Checked;
                    ship.IsWithHelicopter = checkBoxHelicopter.Checked;
                    _ship = ship;
                    break;
            }
