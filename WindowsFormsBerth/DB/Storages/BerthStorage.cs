...
{
    public class BerthStorage
    {
        public List<Berth> GetFullList()
        {
        ...
        public static BerthViewModel GetElement(BerthBindingModel model)
        {
            if (model == null)
            {
                return null;
        ...
        public void Delete(BerthBindingModel model)
        {
            using (var context = new TransportDatabase())
            {
                Berth element = context.Berths.FirstOrDefault(
                    rec => rec.Id == model.Id);
                if (element != null)
                {
